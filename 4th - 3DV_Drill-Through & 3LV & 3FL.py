...
sales_data['Date'] = sales_data['Quarter'].astype(str) + '-' + sales_data['Year'].astype(str)
...
app.layout = html.Div([
    ...
    # Line Visuals
    html.Div([
        dcc.Graph(id='sales-overtime'),
        dcc.Graph(id='sales-overtime-country'),
        dcc.Graph(id='sales-overtime-product')
    ], style={'width': '100%', 'display': 'flex', 'justifyContent': 'space-between'}),
])

# Callbacks for the donut and line visuals
@app.callback(
    [Output('sales-per-year', 'figure'),
     Output('sales-per-country', 'figure'),
     Output('sales-per-product', 'figure'),
     Output('sales-overtime', 'figure'),
     Output('sales-overtime-country', 'figure'),
     Output('sales-overtime-product', 'figure')],
...
def update_visuals(selected_countries, selected_products, selected_years, year_click_data, country_click_data, product_click_data):
    ...
    # Line Visual 2: Sales over time per country
    fig5 = px.line(
        filtered_data.groupby(['Date', 'Country']).sum().reset_index(),
        x='Date',
        y='Sales',
        color='Country',
        title='Sales Over Time Per Country'
    )

    # Line Visual 3: Sales over time per product
    fig6 = px.line(
        filtered_data.groupby(['Date', 'Product']).sum().reset_index(),
        x='Date',
        y='Sales',
        color='Product',
        title='Sales Over Time Per Product'
    )
